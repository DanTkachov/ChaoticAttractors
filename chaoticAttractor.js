...
// const camera = new THREE.PerspectiveCamera( 45, window.innerWidth / window.innerHeight, 10, 1000);
    ...
    for (let i = 0; i < NUM_SPHERES; i++){
        let x = new THREE.SphereGeometry(0.04);
        points.push(x);
    }
    ...
    for(let i =0; i < NUM_SPHERES; i++){
        ...
        // let p = Number("" + row + col)
        // cube.position.set(getRandomInt(100000, 100000), getRandomInt(100000, 100000), 0);
...
function animate() {
    ...
    // swap to delta time
    let time = Date.now()
    requestAnimationFrame( animate ); //calculate
